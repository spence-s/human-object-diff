...
describe('sensitive-paths', () => {
  ...
  test('I can use sensitive in an array', () => {
    ...
    expect(new HR().diff(lhs, rhs)).toEqual([
      '"Bar", with a value of "1" (at Obj.foo[0].bar) was changed to "2"'
    ]);
    expect(new HR({ sensitivePaths: ['foo[].bar'] }).diff(lhs, rhs)).toEqual([
      '"Bar" (at Obj.foo[0].bar) was changed'
    ]);
  });
