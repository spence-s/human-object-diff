...
export function humanReadableDiffGenerator(context: DiffEngineContext): (lhs: unknown, rhs: unknown) => string[] {
  return (lhs: unknown, rhs: unknown): string[] => {
    ...
    for (const singleDeepDiff of differences) {
      ...
      const sentenceDiff = new DiffSentence(diff, context.config, context.templates)
